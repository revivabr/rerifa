-- 1. Redefine reserve_numbers with 3 minutes interval and updated logic
...
BEGIN
  ...
  -- Um número está disponível se:
  -- 1. status = 'available'
  -- 2. status = 'reserved' MAS reserved_until < now()
  ...
  -- Reseta status de números que estavam expirados e agora estão sendo re-reservados por outra pessoa
...
-- 3. Ensure public select on raffle_numbers includes reserved_until for frontend check
-- (Policies already allow select *)
