ALTER TABLE public.buyers ADD COLUMN cpf text;
REVOKE SELECT (cpf) ON public.buyers FROM anon, authenticated;
GRANT SELECT, INSERT, UPDATE (cpf) ON public.buyers TO service_role;
...
BEGIN
  ...
  FOR UPDATE;
  IF v_existing <> v_quantity THEN RETURN json_build_object('ok', false, 'error', 'Um ou mais números não estão disponíveis'); END IF;
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Falha ao reservar números'; END IF;
...
REVOKE ALL ON FUNCTION public.reserve_numbers(uuid,integer[],text,text,text,text) FROM PUBLIC, anon, authenticated;
